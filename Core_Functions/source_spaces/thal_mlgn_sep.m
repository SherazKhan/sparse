...
savename = strcat(dpath,subject,'/src/',subject,'_lt.txt');
if saveon
    save(savename, 'lfull', '-ASCII'); %type(savename);
end

savename = strcat(dpath,subject,'/src/',subject,'_rt.txt');
if saveon
    save(savename, 'rfull', '-ASCII'); %type(savename);
end

load(strcat(dpath, subject, '/src/',subject,'_lt_geom.mat'),'volume','ndipoles');
disp('left'); disp(volume); disp(ndipoles);
volume = (lsz(1)/3)/ndipoles*volume; ndipoles = lsz(1)/3;
if saveon
    save(strcat(dpath, subject, '/src/',subject,'_lt_geom.mat'),'volume','ndipoles');
end
clear volume ndipoles

load(strcat(dpath, subject, '/src/',subject,'_rt_geom.mat'),'volume','ndipoles');
disp('right'); disp(volume); disp(ndipoles);
volume = (rsz(1)/3)/ndipoles*volume; ndipoles = rsz(1)/3;
if saveon
    save(strcat(dpath, subject, '/src/',subject,'_rt_geom.mat'),'volume','ndipoles');
end
