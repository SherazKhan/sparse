...
load(strcat(datapath, '/fwd/', prefix,'_subc_',meastype,'_plr-prewhite','_SVDs',add_on_name,append,'.mat'),'sdiv_fwd'); 
whitened_svd = sdiv_fwd; clear sdiv_fwd;
load(strcat(datapath,'/fwd/',prefix,'_subc_',meastype,'_plr-prewhite_volumes',add_on_name,append,'.mat'));
logFile  = [datapath, '/src/srcspaces_setto_ico',num2str(ico_num), '_details',add_on_name,append,'.txt'];          
mkdir(strcat(datapath,'/mri/svolume_decomps'),append(2:end));

%% Masks to Check Subdivisions in Regions of Interest - All Regions Processed
aseg = MRIread(strcat(datapath,'/mri/aseg.mgz'));
load(strcat(datapath,'/mri/T3.mat')); %T3 = [1 0 0 -0.5; 0 1 0 -0.00; 0 0 1 -0.00; 0 0 0 1.000];
v2ras1 = aseg.vox2ras1; mult = inv(v2ras1)*T3; 
sregion = {'lh','la','lp','lc','lt',...
           'rh','ra','rp','rc','rt',...
           'bs','ic','ic','lhipsurf','rhipsurf'};
...
for i = 1:l
    ...
    if isempty(strfind(regname{i},'surf'))
      ...
      for j = 1:length(ssubdiv{i})
        ...
        c = c+1;
        A = aseg; A.vol = newn;
        MRIwrite(A,strcat(dpath,datatype,'/',prefix,'/mri/svolume_decomps/',append(2:end),'/',regname{i},'_submask',num2str(j),'.mgz'));
     ...
     destName = strcat(datapath, '/mri/svolume_decomps/',append(2:end),'/');%MRI masks for subdivisions
...
fclose(efh);
type_srcspace = repmat({'volume'}, l-2, 1); type_srcspace{l-1} = 'surface'; type_srcspace{l} = 'surface';
