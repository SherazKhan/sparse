...
clear; clc; close all; 
prefix = 'cat_004'; meastype = 'meg_eeg'; datatype = 'AEP';                 %basics: expt, data and subject
...
s = strfind(pwd,'/'); cortpath = pwd; cortpath = strcat(cortpath(1:s(end)), '160805_final_mlr_cortsoln/');
...
cdiv = 3;                                                                   %What cortical subdiv to consider for hybrid source space
...
if select_time == 1                                                         %select known times for evoked response
    ...
    %t1 = 1; t2 = 0.04*Fs;                                                  %1st 30 msec of ERP and 10 msec before stim - t1 set to 10 msec before stim 
end
