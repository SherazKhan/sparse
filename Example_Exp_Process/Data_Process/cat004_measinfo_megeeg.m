function cat004_measinfo_megeeg(prefix, datatype, meastype, datapath, plot_on, append, saveon)
...
include = {};  ear_lobes = {};  exclude = [meas.info.bads ear_lobes]; 	    % include and exclude channels
...
eeg_sel = [eeg_sel(1:54) eeg_sel(55:64)-4];%-(min(eeg_sel)-(max(meg_sel)+1)) = 0;% update EEG Sel for later Use
...
if strcmp(append,'_MLR')
	Ekm = 1; 							    % magnetometer loading
	Ekg = 0;							    % gradiometer loading
else
...
save(savename, 'megYraw','megYp','eegYraw','eegYp','megYw','eegYw','Yraw',...
    ...
    'Fs','Y', 'N', 't1', 't2', 't', 'Leff', 'stc', 'time','scalarX','C','-v7.3');
diary off;
end
