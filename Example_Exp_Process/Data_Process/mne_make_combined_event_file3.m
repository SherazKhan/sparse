function mne_make_combined_event_file3(rawname,eventname,include,all,threshold,delshift,run)
%
...
%   delshift    shift events to account for delay between stimulus src and subject ear
%   run         includes run number - for information on excluding
%   artifacts
...
%   Revision 1.5  2014/01/14 18:50:00 pks
%   Included delshift parameter
%
...
to   = raw.last_samp;
min_time = 0.05;
min_samp = raw.info.sfreq*min_time;
...
for k = 2:size(data,2)
    if comb(k) ~= comb(k-1)
        ...
        if p == 0
            ...
            if samples(k)-events(p,1) > min_samp
                p = p + 1;
                events(p,1:3) = [ samples(k) comb(k-1) comb(k) ];
            end
        end
...
%% Shift Events and Exclude Artifactual Events
events(:,1) = events(:,1) + delshift;
if events(end,1) > to
    events = events(1:end-1,:);
end
if run == 4
    ind = (events(:,1) > 2.5*60*5000-delshift) & (events(:,1) < 3*60*5000+delshift);
    %ind = (events(:,1) < 1.5*60*5000) | ((events(:,1) > 2.5*60*5000-delshift) & (events(:,1) < 3*60*5000+delshift));
    events = events(~ind,:);
end

% if fd >= 0
%     fprintf(1,'Wrote text event file %s (%d events)\n',eventname,q); %#eeents not q
%     fclose(fd);
% end
