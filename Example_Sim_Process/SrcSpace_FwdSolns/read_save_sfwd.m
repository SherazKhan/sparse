...
sregname = {'lh','la','lp','lc','lt',...
            'rh','ra','rp','rc','rt',...
            'bsred','lhipsurf','rhipsurf'};                                 %'bs','lic','ric'};
...
cov_currstr_lim = 0.15;							    %current strength variability
if strcmp(fwd_type,'oct-1')
    set_oct = 1;                                                            %hippocampal srcspace 'oct-2', 'oct-1'
else
...
diary(strcat(prefix, '_ico',num2str(ico_num), '_', meastype, '_srcspace_matlog',append));
...
for i = 1:length(sregname)-2
    ...
    whiteG = Cw*G(meg_sel,:);                                               %Whitened Forward Solution to Consider for Splits and Analyses
    clear fwd ind 
    ...
    if abs(size(srclocs, 1) - size(G,2)) > 0
        ...
        ' --meas $SUBJECTS_DIR/$SUBJECT/meas/$SUBJECT"_SomSens_SingleNoTask_offl_SSP.fif" --mri $SUBJECTS_DIR/$SUBJECT/fwd/$SUBJECT"-trans.fif"',...
        ' --bem $SUBJECTS_DIR/$SUBJECT/bem/$SUBJECT"-5120-5120-5120-bem-sol.fif" --all --fwd $SUBJECTS_DIR/$SUBJECT/fwd/$SUBJECT"_"$reg"_meg-fwd.fif"']);
        pause;
        fwd_nomindist = mne_read_forward_solution(strcat(datapath, '/fwd/', prefix, '_',reg_name, '_meg-fwd.fif'));
        src = mne_read_source_spaces('/autofs/eris/purdongp/users/pavitrak/sourceloc_data/',datatype,'/',prefix,'/src/',prefix,'_bs-src.fif');
        %[~, lk, ~] = intersect(src.vertno',fwd_nomindist.src.vertno','rows');%indices of sources that are excluded in fwd_nomindist due to dist. inner skull
        [~, lk, ~] = intersect(src.vertno',fwd.src.vertno','rows');         %G is based on fwd.source_Rr ==> select srclocs with same source locations
        %lk = fwd_nomindist.src.vertno(logical(fwd.src.inuse));             %ID Vertices that are in both Fwd and Mindist Fwd
    ...
    cort_currstr_mean, subc_cd(i), V(i), ndipoles, geom_currstr, perc_match, cov_currstr_lim, Cw, meg_sel);
...
save(strcat(datapath,'/fwd/',prefix,'_subc_',meastype,'_prewhite_volumes',add_on_name,append,'.mat'),...
