...
if fullcort == 0
    scort_resfname = strcat(pwd,'/',append(2:end),'_hip-',hfwd_type,'/', prefix,'-',meastype,'-',datatype,'_scortest',append,'.mat');       % subcortical results
else
    scort_resfname = strcat(pwd,'/',append(2:end),'_hip-',hfwd_type,'/', prefix,'-',meastype,'-',datatype,'_1stage_scortest',append,'.mat');% subcortical results
end
...
if fullcort == 0
    rois = {'bsred-5','ra-9','lc-18','rc-5','lc-12','rc-24','lc-5','la-2'}; % HFABR Manny Results (2-stage)
else
    rois = {'l-other1', 'l-other1', 'l-other1', 'l-other2', 'l-other1', 'r-other1', 'r-other1', 'r-other1'};  %HFABR Manny Results (1-stage)
end
disp(rois);
...
if fullcort 
   ...
   for i = 1:length(cort_roi_index)
    ...
    cort_rois = {'la1','ra1','ra1'};
    cort_roi_index = {1, 1, 2};
end
...
Xls_plot_mnecond_gr, Xls_plot_mnecond_all, time_est, T, unique_rois, legs_mne_condall, col, thresh, fullcort, sel_times); 
...
currdistrn_rois(regions_list, n_greedy, leg_greedy, 'scsp', thresh2, fullcort, strcat(regname,'-')); %bar graph for greedy estimates
currdistrn_rois(regions_list, n_mne, leg_mne,'mne-sel', thresh2, fullcort, strcat(regname,'-'));     %bar graph for mne estimates in selected regions
currdistrn_rois(regions_list, n_mne_all, leg_mne_all, 'mne', thresh2, fullcort, strcat(regname,'-'));%bar graph for all mne estimates
