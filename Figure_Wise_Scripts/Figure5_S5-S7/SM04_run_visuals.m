...
mne_mask_en = 84;                                                          % end point in milliseconds
else
...
respath = '/autofs/eris/purdongp/users/pavitrak/sourceloc_scripts/SM04-code/inv-SM04_SEP/160930_final_SM04_scortsoln_ico3';
if fullcort == 0
    scort_resfname = strcat(respath,'/',append(2:end),'_final/', prefix,'-',meastype,'-',datatype,'_scortest',append,'.mat');       % subcortical results file
else
    scort_resfname = strcat(respath,'/',append(2:end),'_final/', prefix,'-',meastype,'-',datatype,'_1stage_scortest',append,'.mat');% subcortical results file
end
...
if fullcort 
   ...
   for i = 1:length(cort_roi_index)
       ...
       lblname =  strcat(num2str(clblno(i),'%06g'),'-',clr(i),'h.label');
       cort_rois{i} = lblname;                                              %cortical label file name
   end
else
    if strcmp(append,'_overlap')
...
Xls_plot_mnecond_all, time_est, T, unique_rois, unique_cort_scort, legs_mne_condall, cort_scort_mne, col, nthresh, fullcort, sel_times, sim_on); 
...
if fullcort == 0
    ...
    leg_mne_all(ismember(leg_mne_all, {'038477-lh.label','047321-lh.label','058397-lh.label'})) = {'cs1'}; %cortical label file name
    leg_mne_all(ismember(leg_mne_all, {'038344-lh.label','035853-lh.label','028238-lh.label',....
        '029479-lh.label','026646-lh.label','028022-lh.label'})) = {'ppc'}; %cortical label file name
    leg_mne_all(ismember(leg_mne_all, {'053692-lh.label','054919-lh.label'})) = {'cs2'}; %cortical label file name
    figure(4), legend(leg_mne_all);
    regions_list = [leg_mne_all, 'is2',strcat(regname,'-')];                %select cortical (based on shortlist from MNE) & all subcort regions
    cort_scort_flag = [ones(1,length(leg_mne_all)),zeros(1, 1+length(regname))];
end
leg_greedy = strcat(leg_greedy,'-');                                        %so that comparisons are possible with updated regions list
currdistrn_rois(regions_list, n_sim, leg_sim, 'sim', n_greedy, leg_greedy, 'scsp',cort_scort_flag, 1);%bar graph for greedy estimates
currdistrn_rois(regions_list, n_sim, leg_sim, 'sim', n_mne, leg_mne, 'mne-sel', cort_scort_flag, 0);%bar graph for mne estimates in selected regions
currdistrn_rois(regions_list, n_sim, leg_sim, 'sim', n_mne_all, leg_mne_all, 'mne',cort_scort_flag, 0);%bar graph for all mne estimates
